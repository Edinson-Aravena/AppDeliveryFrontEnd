...
interface Props extends StackScreenProps<ProductStackParamList, 'RestaurantProductUpdateScreen'>{};

export const RestaurantProductUpdateScreen = ({navigation, route}:Props) => {

    const {category, product} = route.params;
    const { name, description, image1, image2, image3, price, loading, responseMessage, onChange, takePhoto, pickImage, updateProduct } = useViewModel(product, category);
            ...
            <View style={styles.form}>
                <ScrollView>
                    ...
                    <View style={styles.containerCategory}>
                        ...
                        <Text style={{ fontSize: 20, color: globalColors.info}}>Producto: </Text>
                        <Text style={{ fontSize: 15, color: globalColors.info, fontWeight: 'bold'  }}>{product.name}</Text>
                    </View>

                    <View>
                        <RoundedButtonComponent text='ACTUALIZAR PRODUCTO' onPress={() => {
                            updateProduct()
                        }} />
                    </View>
